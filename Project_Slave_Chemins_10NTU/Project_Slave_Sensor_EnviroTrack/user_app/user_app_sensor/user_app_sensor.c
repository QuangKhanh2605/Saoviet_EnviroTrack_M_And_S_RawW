...
Struct_Sensor_TURB  sSensor_TURB ={0};
...
//    {
//        case _LCD_SCR_SET_CALIB_SS_TURB:
...
//          else
//          {
//            sKindMode485.Trans = _RS485_SS_TURB_OPERA;
//          }
//          break;
//          
//        default:
...
static uint8_t fevent_temp_alarm(uint8_t event)
{
    if(sTempAlarm.State == 1 && sHandleRs485.State_Recv_TURB == 1)
    {
        if((sSensor_TURB.TURB_Filter_f >= sTempAlarm.Alarm_Upper) || 
           (sSensor_TURB.TURB_Filter_f < sTempAlarm.Alarm_Lower))
        {
...
static uint8_t fevent_handle_state_sensor(uint8_t event)
{
    if(sSensor_TURB.State_Connect == _SENSOR_DISCONNECT)
        sSensor_TURB.State_Sensor_u8 = _SS_DISCONNECT;
    else
    {
        if((sLCD.sScreenNow.Index_u8 == _LCD_SCR_SET_CALIB_SS_TURB) || 
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_CHECK_SETTING && sLCD.sScreenBack.Index_u8 == _LCD_SCR_SET_CALIB_SS_TURB))
            sSensor_TURB.State_Sensor_u8 = _SS_CALIB;
        else
            sSensor_TURB.State_Sensor_u8 = _SS_MEASURE;
    }
    
    if(sHandleRs485.State_Recv_TURB == 1)
    {
        sSensor_TURB.State_Measure_TURB_u8 = _MEASURE_VALID;
        sSensor_TURB.State_Measure_Temp_u8 = _MEASURE_VALID;
    }
    else
    {
        sSensor_TURB.State_Measure_TURB_u8 = _MEASURE_INVALID;
        sSensor_TURB.State_Measure_Temp_u8 = _MEASURE_INVALID;
    }
...
void Handle_Data_Measure(uint8_t KindRecv)
{
    ...
    switch(KindRecv)
    {
        case _RS485_SS_TURB_OPERA:
            ...
            sSensor_TURB.TURB_Value_f = quickSort_Sampling_Value((int32_t)(sSensor_TURB.TURB_Value_f*1000));
            
            sSensor_TURB.TURB_Filter_f = Filter_pH(sSensor_TURB.TURB_Value_f);
            sSensor_TURB.temp_Filter_f = Filter_Temp(sSensor_TURB.temp_Value_f);
            
            Stamp = sSensor_TURB.temp_Filter_f + sSensor_TURB.temp_Offset_f;
            sSensor_TURB.temp_Filter_f = ((Stamp) > sMeasureRange.Upper_Temp) ? sMeasureRange.Upper_Temp : ((Stamp) < sMeasureRange.Lower_Temp ? sMeasureRange.Lower_Temp : (Stamp));
            
            Stamp = sSensor_TURB.TURB_Filter_f + sSensor_TURB.TURB_Offset_f;
            sSensor_TURB.TURB_Filter_f = ((Stamp) > sMeasureRange.Upper_Key) ? sMeasureRange.Upper_Key : ((Stamp) < sMeasureRange.Lower_Key ? sMeasureRange.Lower_Key : (Stamp));
          break;
...
void Handle_Data_Trans_SS_TURB(sData *sFrame, uint8_t KindTrans)
{
    switch(KindTrans)
    {
        //Trans Opera
        case _RS485_SS_TURB_OPERA:
            ModRTU_Master_Read_Frame(sFrame, ID_DEFAULT_SS_TURB, 0x03, 0x0000, 0x04);
            break;
            
        case _RS485_SS_TURB_ZERO:
            ModRTU_Master_Read_Frame(sFrame, ID_DEFAULT_SS_TURB, 0x03, 0x1000, 0x01);
            break;
            
        case _RS485_SS_TURB_SLOPE:
            ModRTU_Master_Read_Frame(sFrame, ID_DEFAULT_SS_TURB, 0x03, 0x1004, 0x01);
            break;
            
         //Trans Calib
        case _RS485_SS_TURB_CALIB_ZERO:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_TURB, 0x06, 0x1000, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
            
        case _RS485_SS_TURB_CALIB_SLOPE:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_TURB, 0x06, 0x1004, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
            
        case _RS485_SS_TURB_RESET:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_TURB, 0x06, 0x2020, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
...
void Handle_Data_Recv_SS_TURB(sData sDataRS485, uint8_t KindRecv)
{
    ...
    switch(KindRecv)
    {
        ...
        case _RS485_SS_TURB_OPERA:
            ...
            sSensor_TURB.TURB_Value_f =  ((float)Stamp_Hex)/(pow(10,Decimal_Hex));
            
            Stamp_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2);
            Decimal_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2); 
            sSensor_TURB.temp_Value_f =  ((float)Stamp_Hex)/(pow(10,Decimal_Hex));
          break;
          
        case _RS485_SS_TURB_ZERO:
            Pos = 3;
            Stamp_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2);
            sSensor_TURB.TURB_Zero_Calib_f = ((float)Stamp_Hex)/10;
          break;
          
        case _RS485_SS_TURB_SLOPE:
            Pos = 3;
            Stamp_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2);
            sSensor_TURB.TURB_Slope_Calib_f = ((float)Stamp_Hex)/10;
          break;
...
void Handle_State_SS_TURB(uint8_t KindRecv, uint8_t KindDetect)
{
    switch(KindRecv)
    {
        ...
        case _RS485_SS_TURB_RESET:
          ...
          {
            sSensor_TURB.Count_Disconnect = 0;
            sSensor_TURB.State_Connect = _SENSOR_CONNECT;
            
            if(KindRecv == _RS485_SS_TURB_OPERA)
              sHandleRs485.State_Recv_TURB = 1;
          }
          else
          {
            if(sSensor_TURB.Count_Disconnect < 3)
                sSensor_TURB.Count_Disconnect++;
          }
          break;
    ...
    if(sSensor_TURB.Count_Disconnect == 0)
    {
        
    }
    else if(sSensor_TURB.Count_Disconnect >=3)
    {
        sSensor_TURB.State_Connect = _SENSOR_DISCONNECT;
        sSensor_TURB.TURB_Value_f = 0;
        sSensor_TURB.temp_Value_f = 0;
        
        sSensor_TURB.TURB_Zero_Calib_f = 0;
        sSensor_TURB.TURB_Slope_Calib_f = 0;

        sHandleRs485.State_Recv_TURB = 0;
    }
}
/*====================Filter Data===================*/
...
void Save_ParamCalib(float TURB_Offset_f, float temp_Offset_f)
{
    ...
    sSensor_TURB.TURB_Offset_f = TURB_Offset_f;
    sSensor_TURB.temp_Offset_f = temp_Offset_f;
    
    hexUint_Compensation_TURB = Handle_Float_To_hexUint32(sSensor_TURB.TURB_Offset_f);
    hexUint_Compensation_temp = Handle_Float_To_hexUint32(sSensor_TURB.temp_Offset_f);
...
void Init_ParamCalib(void)
{
    ...
    {
        hexUint_Compensation_TURB  = *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+2) << 24;
        hexUint_Compensation_TURB  |= *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+3)<< 16;
        hexUint_Compensation_TURB  |= *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+4)<< 8;
        hexUint_Compensation_TURB  |= *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+5);
        ...
        Convert_uint32Hex_To_Float(hexUint_Compensation_TURB,  &sSensor_TURB.TURB_Offset_f);
        Convert_uint32Hex_To_Float(hexUint_Compensation_temp, &sSensor_TURB.temp_Offset_f);
    }
    else
    {
        sSensor_TURB.TURB_Offset_f = 0;
        sSensor_TURB.temp_Offset_f = 0;
    }
#endif   
}
