...
//    }
    ...
    {
        sKindMode485.Trans = _RS485_SS_CLO_OPERA;
    }
...
static uint8_t fevent_temp_alarm(uint8_t event)
{
    if(sTempAlarm.State == 1 && sHandleRs485.State_Recv_CLO == 1)
    {
        if((sSensor_CLO.CLO_Filter_f >= sTempAlarm.Alarm_Upper) || 
           (sSensor_CLO.CLO_Filter_f < sTempAlarm.Alarm_Lower))
        {
...
static uint8_t fevent_handle_state_sensor(uint8_t event)
{
    if(sSensor_CLO.State_Connect == _SENSOR_DISCONNECT)
        sSensor_CLO.State_Sensor_u8 = _SS_DISCONNECT;
    else
    {
        if((sLCD.sScreenNow.Index_u8 == _LCD_SCR_SET_CALIB_SS_CLO) || 
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_CHECK_SETTING && sLCD.sScreenBack.Index_u8 == _LCD_SCR_SET_CALIB_SS_CLO))
            sSensor_CLO.State_Sensor_u8 = _SS_CALIB;
        else
            sSensor_CLO.State_Sensor_u8 = _SS_MEASURE;
    }
    
    if(sHandleRs485.State_Recv_CLO == 1)
    {
        sSensor_CLO.State_Measure_CLO_u8 = _MEASURE_VALID;
        sSensor_CLO.State_Measure_Temp_u8 = _MEASURE_VALID;
    }
    else
    {
        sSensor_CLO.State_Measure_CLO_u8 = _MEASURE_INVALID;
        sSensor_CLO.State_Measure_Temp_u8 = _MEASURE_INVALID;
    }
...
void Handle_Data_Measure(uint8_t KindRecv)
{
    ...
    switch(KindRecv)
    {
        case _RS485_SS_CLO_OPERA:
            ...
            sSensor_CLO.CLO_Value_f = quickSort_Sampling_Value((int32_t)(sSensor_CLO.CLO_Value_f*1000));
            
            sSensor_CLO.CLO_Filter_f = Filter_pH(sSensor_CLO.CLO_Value_f);
            sSensor_CLO.temp_Filter_f = Filter_Temp(sSensor_CLO.temp_Value_f);
            
            Stamp = sSensor_CLO.temp_Filter_f + sSensor_CLO.temp_Offset_f;
            sSensor_CLO.temp_Filter_f = ((Stamp) > sMeasureRange.Upper_Temp) ? sMeasureRange.Upper_Temp : ((Stamp) < sMeasureRange.Lower_Temp ? sMeasureRange.Lower_Temp : (Stamp));
            
            Stamp = sSensor_CLO.CLO_Filter_f + sSensor_CLO.CLO_Offset_f;
            sSensor_CLO.CLO_Filter_f = ((Stamp) > sMeasureRange.Upper_Key) ? sMeasureRange.Upper_Key : ((Stamp) < sMeasureRange.Lower_Key ? sMeasureRange.Lower_Key : (Stamp));
          break;
...
void Handle_Data_Trans_SS_CLO(sData *sFrame, uint8_t KindTrans)
{
    switch(KindTrans)
    {
        //Trans Opera
        case _RS485_SS_CLO_OPERA:
            ModRTU_Master_Read_Frame(sFrame, ID_DEFAULT_SS_CLO, 0x03, 0x0000, 0x04);
            break;
            
         //Trans Calib
        case _RS485_SS_CLO_CALIB_ZERO:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_CLO, 0x06, 0x1000, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
            
        case _RS485_SS_CLO_CALIB_SLOPE:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_CLO, 0x06, 0x1004, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
            
        case _RS485_SS_CLO_RESET:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_CLO, 0x06, 0x2020, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
...
void Handle_Data_Recv_SS_CLO(sData sDataRS485, uint8_t KindRecv)
{
    ...
    switch(KindRecv)
    {
        ...
        case _RS485_SS_CLO_OPERA:
            ...
            sSensor_CLO.CLO_Value_f =  ((float)Stamp_Hex)/(pow(10,Decimal_Hex));
            
            Stamp_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2);
            Decimal_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2); 
            sSensor_CLO.temp_Value_f =  ((float)Stamp_Hex)/(pow(10,Decimal_Hex));
          break;
