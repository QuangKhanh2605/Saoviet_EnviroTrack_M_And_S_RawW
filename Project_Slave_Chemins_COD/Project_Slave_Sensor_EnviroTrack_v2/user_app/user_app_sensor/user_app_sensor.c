...
static uint8_t fevent_temp_alarm(uint8_t event)
{
    if(sTempAlarm.State == 1 && sHandleRs485.State_Recv_COD == 1)
    {
        if((sSensor_COD.COD_Filter_f >= sTempAlarm.Alarm_Upper) || 
           (sSensor_COD.COD_Filter_f < sTempAlarm.Alarm_Lower))
        {
...
static uint8_t fevent_handle_state_sensor(uint8_t event)
{
    if(sSensor_COD.State_Connect == _SENSOR_DISCONNECT)
        sSensor_COD.State_Sensor_u8 = _SS_DISCONNECT;
    else
    {
        if((sLCD.sScreenNow.Index_u8 == _LCD_SCR_SET_CALIB_SS_COD) || 
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_CHECK_SETTING && sLCD.sScreenBack.Index_u8 == _LCD_SCR_SET_CALIB_SS_COD) ||
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_SET_CALIB_SS_COD_PLS) || 
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_CHECK_SETTING && sLCD.sScreenBack.Index_u8 == _LCD_SCR_SET_CALIB_SS_COD_PLS))
            sSensor_COD.State_Sensor_u8 = _SS_CALIB;
        else
            sSensor_COD.State_Sensor_u8 = _SS_MEASURE;
    }
    
    if(sHandleRs485.State_Recv_COD == 1)
    {
        sSensor_COD.State_Measure_COD_u8 = _MEASURE_VALID;
        sSensor_COD.State_Measure_Temp_u8 = _MEASURE_VALID;
    }
    else
    {
        sSensor_COD.State_Measure_COD_u8 = _MEASURE_INVALID;
        sSensor_COD.State_Measure_Temp_u8 = _MEASURE_INVALID;
    }
...
void Handle_Data_Measure(uint8_t KindRecv)
{
    ...
    switch(KindRecv)
    {
        case _RS485_SS_COD_OPERA:
            ...
            sCalibPlus.var_x_f = sSensor_COD.COD_Value_f;
            sCalibPlus.var_y_f = Cal_CalibPlus(sCalibPlus.var_x_f);
            sSensor_COD.COD_Value_f = sCalibPlus.var_y_f;
            ...
            Stamp = sSensor_COD.temp_Filter_f + sSensor_COD.temp_Offset_f;
            sSensor_COD.temp_Filter_f = ((Stamp) > sMeasureRange.Upper_Temp) ? sMeasureRange.Upper_Temp : ((Stamp) < sMeasureRange.Lower_Temp ? sMeasureRange.Lower_Temp : (Stamp));
            
            sSensor_COD.TURB_Filter_f += sSensor_COD.TURB_Offset_f;
            
            Stamp = sSensor_COD.COD_Filter_f + sSensor_COD.COD_Offset_f;
            sSensor_COD.COD_Filter_f = ((Stamp) > sMeasureRange.Upper_Key) ? sMeasureRange.Upper_Key : ((Stamp) < sMeasureRange.Lower_Key ? sMeasureRange.Lower_Key : (Stamp));
          break;
