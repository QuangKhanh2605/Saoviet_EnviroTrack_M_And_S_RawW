...
static uint8_t fevent_handle_state_sensor(uint8_t event)
{
    if(sSensor_DO.State_Connect == _SENSOR_DISCONNECT)
        sSensor_DO.State_Sensor_u8 = _SS_DISCONNECT;
    else
    {
        if((sLCD.sScreenNow.Index_u8 == _LCD_SCR_SET_CALIB_SS_DO) || 
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_CHECK_SETTING && sLCD.sScreenBack.Index_u8 == _LCD_SCR_SET_CALIB_SS_DO) ||
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_SET_CALIB_SS_DO_PLS) || 
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_CHECK_SETTING && sLCD.sScreenBack.Index_u8 == _LCD_SCR_SET_CALIB_SS_DO_PLS))
            sSensor_DO.State_Sensor_u8 = _SS_CALIB;
        else
            sSensor_DO.State_Sensor_u8 = _SS_MEASURE;
    }
    
    if(sHandleRs485.State_Recv_DO == 1)
    {
        sSensor_DO.State_Measure_DO_u8 = _MEASURE_VALID;
        sSensor_DO.State_Measure_Temp_u8 = _MEASURE_VALID;
    }
    else
    {
        sSensor_DO.State_Measure_DO_u8 = _MEASURE_INVALID;
        sSensor_DO.State_Measure_Temp_u8 = _MEASURE_INVALID;
    }
...
void Handle_Data_Measure(uint8_t KindRecv)
{
    ...
    switch(KindRecv)
    {
        case _RS485_SS_DO_OPERA:
            ...
            sCalibPlus.var_x_f = sSensor_DO.Oxy_Mg_L_Value_f;
            sCalibPlus.var_y_f = Cal_CalibPlus(sCalibPlus.var_x_f);
            sSensor_DO.Oxy_Mg_L_Value_f = sCalibPlus.var_y_f;
            ...
            Stamp = sSensor_DO.temp_Filter_f + sSensor_DO.temp_Offset_f;
            sSensor_DO.temp_Filter_f = ((Stamp) > sMeasureRange.Upper_Temp) ? sMeasureRange.Upper_Temp : ((Stamp) < sMeasureRange.Lower_Temp ? sMeasureRange.Lower_Temp : (Stamp));
            
            Stamp = sSensor_DO.Oxy_Mg_L_Filter_f + sSensor_DO.Oxy_Mg_L_Offset_f;
            sSensor_DO.Oxy_Mg_L_Filter_f = ((Stamp) > sMeasureRange.Upper_Key) ? sMeasureRange.Upper_Key : ((Stamp) < sMeasureRange.Lower_Key ? sMeasureRange.Lower_Key : (Stamp));
          break;
