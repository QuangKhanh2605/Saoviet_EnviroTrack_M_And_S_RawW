...
//    }
    ...
    if(sKindMode485.Trans > _RS485_SS_EC_OPERA)
    {
        sKindMode485.Trans = _RS485_SS_EC_OPERA;
    }
...
/*------------------Xu ly chuoi nhan duoc----------------*/
    ...
    if(sUart485SS.Length_u16 > 2)
    {
        ...
        if(Crc_Check == Crc_Recv)
        {
            ...
            if(sUart485SS.Data_a8[0] == ID_DEFAULT_SS_EC)
            {
...
static uint8_t fevent_handle_state_sensor(uint8_t event)
{
    ...
    {
        sSensor_EC.State_Measure_EC_u8 = _MEASURE_VALID;
        sSensor_EC.State_Measure_Temp_u8 = _MEASURE_VALID;
    }
    else
    {
        sSensor_EC.State_Measure_EC_u8 = _MEASURE_INVALID;
...
void Handle_Data_Measure(uint8_t KindRecv)
{
    ...
    switch(KindRecv)
    {
        case _RS485_SS_EC_OPERA:
            ...
            sSensor_EC.EC_Value_f = quickSort_Sampling_Value((int32_t)(sSensor_EC.EC_Value_f*100));
            ...
            sSensor_EC.EC_Filter_f = ((Stamp) > sMeasureRange.Upper_Key) ? sMeasureRange.Upper_Key : ((Stamp) < sMeasureRange.Lower_Key ? sMeasureRange.Lower_Key : (Stamp));
          break;
    ...
    if(sSensor_EC.State_Connect == _SENSOR_DISCONNECT)
    {
        sSensor_EC.temp_Value_f = 0;
        sSensor_EC.temp_Filter_f = Filter_Temp(sSensor_EC.temp_Value_f);;
    }
    
    sSensor_EC.TDS_Value_f = sSensor_EC.EC_Value_f * sSensor_EC.ConvertFactor_ECtoTDS;
...
void Handle_Data_Trans_SS_EC(sData *sFrame, uint8_t KindTrans)
{
    switch(KindTrans)
    {
        //Trans Opera
        case _RS485_SS_EC_OPERA:
            ModRTU_Master_Read_Frame(sFrame, ID_DEFAULT_SS_EC, 0x03, 0x0000, 0x06);
            break;
            
         //Trans Calib
        case _RS485_SS_EC_CALIB_ZERO:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_EC, 0x06, 0x1000, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
            
        case _RS485_SS_EC_CALIB_SLOPE:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_EC, 0x06, 0x1004, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
            
        case _RS485_SS_EC_RESET:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_EC, 0x06, 0x2020, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
...
void Handle_Data_Recv_SS_EC(sData sDataRS485, uint8_t KindRecv)
{
    ...
    switch(KindRecv)
    {
        ...
        case _RS485_SS_EC_OPERA:
            ...
            sSensor_EC.EC_Value_f =  ((float)Stamp_Hex)/(pow(10,Decimal_Hex));
            
            Stamp_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2);
            Decimal_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2); 
            sSensor_EC.temp_Value_f =  ((float)Stamp_Hex)/(pow(10,Decimal_Hex));
          break;
...
/*===================Save and Init Calib====================*/
void Save_ParamCalib(float EC_Offset_f, float temp_Offset_f)
{
    ...
    sSensor_EC.EC_Offset_f = EC_Offset_f;
    sSensor_EC.temp_Offset_f = temp_Offset_f;
    
    hexUint_Compensation_EC    = Handle_Float_To_hexUint32(sSensor_EC.EC_Offset_f);
    hexUint_Compensation_temp   = Handle_Float_To_hexUint32(sSensor_EC.temp_Offset_f);
...
void Init_ParamCalib(void)
{
    ...
    {
        hexUint_Compensation_EC  = *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+2) << 24;
        hexUint_Compensation_EC  |= *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+3)<< 16;
        hexUint_Compensation_EC  |= *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+4)<< 8;
        hexUint_Compensation_EC  |= *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+5);
        ...
        Convert_uint32Hex_To_Float(hexUint_Compensation_EC,  &sSensor_EC.EC_Offset_f );
        Convert_uint32Hex_To_Float(hexUint_Compensation_temp, &sSensor_EC.temp_Offset_f);
    }
    else
    {
        sSensor_EC.EC_Offset_f = 0;
        sSensor_EC.temp_Offset_f = 0;
    }
#endif   
}
...
void Save_ConvertFactorEC(float Factor)
{
    ...
    sSensor_EC.ConvertFactor_ECtoTDS = Factor;
    
    hexUint  = Handle_Float_To_hexUint32(sSensor_EC.ConvertFactor_ECtoTDS);
    
    aData[length++] = sTempAlarm.State;
    
    aData[length++] = hexUint >> 24;
    aData[length++] = hexUint >> 16;
    aData[length++] = hexUint >> 8;
    aData[length++] = hexUint ;

    Save_Array(ADDR_CONVERT_FACTOR_EC, aData, length);
#endif   
}
...
void Init_ConvertFactorEC(void)
{
    ...
    if(*(__IO uint8_t*)(ADDR_CONVERT_FACTOR_EC) != FLASH_BYTE_EMPTY)
    {
        sTempAlarm.State  = *(__IO uint8_t*)(ADDR_CONVERT_FACTOR_EC+2);
      
        hexUint  = *(__IO uint8_t*)(ADDR_CONVERT_FACTOR_EC+3) << 24;
        hexUint  |= *(__IO uint8_t*)(ADDR_CONVERT_FACTOR_EC+4)<< 16;
        hexUint  |= *(__IO uint8_t*)(ADDR_CONVERT_FACTOR_EC+5)<< 8;
        hexUint  |= *(__IO uint8_t*)(ADDR_CONVERT_FACTOR_EC+6);
        
        Convert_uint32Hex_To_Float(hexUint, &sSensor_EC.ConvertFactor_ECtoTDS);
    }
    else
    {
        sSensor_EC.ConvertFactor_ECtoTDS = 0.64;
    }
#endif   
}
