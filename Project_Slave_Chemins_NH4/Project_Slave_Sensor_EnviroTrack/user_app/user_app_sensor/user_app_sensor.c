...
//    }
    ...
    if(sKindMode485.Trans > _RS485_SS_NH4_OPERA)
    {
...
static uint8_t fevent_handle_state_sensor(uint8_t event)
{
    ...
    {
        if((sLCD.sScreenNow.Index_u8 == _LCD_SCR_SET_CALIB_SS_NH4) || 
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_CHECK_SETTING && sLCD.sScreenBack.Index_u8 == _LCD_SCR_SET_CALIB_SS_NH4))
            sSensor_NH4.State_Sensor_u8 = _SS_CALIB;
        else
            sSensor_NH4.State_Sensor_u8 = _SS_MEASURE;
    }
    
    if(sHandleRs485.State_Recv_NH4 == 1)
    {
...
void Handle_Data_Trans_SS_NH4(sData *sFrame, uint8_t KindTrans)
{
    switch(KindTrans)
    {
        ...
        case _RS485_SS_NH4_CALIB_SLOPE:
            if(sData_Calib.Length_u16 == 2)
...
void Handle_Data_Recv_SS_NH4(sData sDataRS485, uint8_t KindRecv)
{
    ...
    switch(KindRecv)
    {
        ...
        case _RS485_SS_NH4_OPERA:
            ...
            sSensor_NH4.NH4_Value_f =  ((float)Stamp_Hex)/(pow(10,Decimal_Hex));
...
void Save_ParamCalib(float NH4_Offset_f, float temp_Offset_f)
{
    ...
    hexUint_Compensation_NH4    = Handle_Float_To_hexUint32(sSensor_NH4.NH4_Offset_f);
...
void Init_ParamCalib(void)
{
    ...
    {
        hexUint_Compensation_NH4  = *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+2) << 24;
        hexUint_Compensation_NH4  |= *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+3)<< 16;
        hexUint_Compensation_NH4  |= *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+4)<< 8;
        hexUint_Compensation_NH4  |= *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+5);
        ...
        Convert_uint32Hex_To_Float(hexUint_Compensation_NH4,  &sSensor_NH4.NH4_Offset_f );
