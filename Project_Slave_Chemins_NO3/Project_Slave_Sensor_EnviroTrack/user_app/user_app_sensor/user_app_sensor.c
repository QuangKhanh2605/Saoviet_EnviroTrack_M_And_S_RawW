...
Struct_Sensor_NO3    sSensor_NO3={0};
...
//    }
    ...
    if(sKindMode485.Trans > _RS485_SS_NO3_OPERA)
    {
        sKindMode485.Trans = _RS485_SS_NO3_OPERA;
    }
...
/*------------------Xu ly chuoi nhan duoc----------------*/
    ...
    if(sUart485SS.Length_u16 > 2)
    {
        ...
        if(Crc_Check == Crc_Recv)
        {
            ...
            if(sUart485SS.Data_a8[0] == ID_DEFAULT_SS_NO3)
            {
...
static uint8_t fevent_temp_alarm(uint8_t event)
{
    if(sTempAlarm.State == 1 && sHandleRs485.State_Recv_NO3 == 1)
    {
        if((sSensor_NO3.NO3_Filter_f >= sTempAlarm.Alarm_Upper) || 
           (sSensor_NO3.NO3_Filter_f < sTempAlarm.Alarm_Lower))
        {
...
static uint8_t fevent_handle_state_sensor(uint8_t event)
{
    if(sSensor_NO3.State_Connect == _SENSOR_DISCONNECT)
        sSensor_NO3.State_Sensor_u8 = _SS_DISCONNECT;
    else
    {
        if((sLCD.sScreenNow.Index_u8 == _LCD_SCR_SET_CALIB_SS_NO3) || 
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_CHECK_SETTING && sLCD.sScreenBack.Index_u8 == _LCD_SCR_SET_CALIB_SS_NO3))
            sSensor_NO3.State_Sensor_u8 = _SS_CALIB;
        else
            sSensor_NO3.State_Sensor_u8 = _SS_MEASURE;
    }
    
    if(sHandleRs485.State_Recv_NO3 == 1)
    {
        sSensor_NO3.State_Measure_NO3_u8 = _MEASURE_VALID;
        sSensor_NO3.State_Measure_Temp_u8 = _MEASURE_VALID;
    }
    else
    {
        sSensor_NO3.State_Measure_NO3_u8 = _MEASURE_INVALID;
        sSensor_NO3.State_Measure_Temp_u8 = _MEASURE_INVALID;
    }
...
void Handle_Data_Measure(uint8_t KindRecv)
{
    ...
    switch(KindRecv)
    {
        case _RS485_SS_NO3_OPERA:
            
            sSensor_NO3.NO3_Value_f = quickSort_Sampling_Value((int32_t)(sSensor_NO3.NO3_Value_f*100));
            
            sSensor_NO3.NO3_Filter_f = Filter_pH(sSensor_NO3.NO3_Value_f);
            sSensor_NO3.temp_Filter_f = Filter_Temp(sSensor_NO3.temp_Value_f);
            
            Stamp = sSensor_NO3.temp_Filter_f + sSensor_NO3.temp_Offset_f;
            sSensor_NO3.temp_Filter_f = ((Stamp) > sMeasureRange.Upper_Temp) ? sMeasureRange.Upper_Temp : ((Stamp) < sMeasureRange.Lower_Temp ? sMeasureRange.Lower_Temp : (Stamp));
            
            Stamp = sSensor_NO3.NO3_Filter_f + sSensor_NO3.NO3_Offset_f;
            sSensor_NO3.NO3_Filter_f = ((Stamp) > sMeasureRange.Upper_Key) ? sMeasureRange.Upper_Key : ((Stamp) < sMeasureRange.Lower_Key ? sMeasureRange.Lower_Key : (Stamp));
          break;
    ...
    if(sSensor_NO3.State_Connect == _SENSOR_DISCONNECT)
    {
        sSensor_NO3.NO3_Value_f = 0;
        sSensor_NO3.NO3_Filter_f = Filter_pH(sSensor_NO3.NO3_Value_f);
    }
    
    if(sSensor_NO3.State_Connect == _SENSOR_DISCONNECT)
    {
        sSensor_NO3.temp_Value_f = 0;
        sSensor_NO3.temp_Filter_f = Filter_Temp(sSensor_NO3.temp_Value_f);;
    }
}
...
{
    Handle_Data_Trans_SS_NO3(sFrame, KindRecv);
}
void Handle_Data_Recv_Sensor(sData sDataRS485, uint8_t KindRecv)
{
    if(sDataRS485.Data_a8[0] == ID_DEFAULT_SS_NO3)
        Handle_Data_Recv_SS_NO3(sDataRS485, KindRecv);
}

void Handle_State_Sensor(uint8_t KindRecv, uint8_t KindDetect)
{
    Handle_State_SS_NO3(KindRecv, KindDetect);
}
/*==================Handle Sensor NO3===================*/
void Handle_Data_Trans_SS_NO3(sData *sFrame, uint8_t KindTrans)
{
    switch(KindTrans)
    {
        //Trans Opera
        case _RS485_SS_NO3_OPERA:
            ModRTU_Master_Read_Frame(sFrame, ID_DEFAULT_SS_NO3, 0x03, 0x0000, 0x04);
            break;
            
         //Trans Calib
        case _RS485_SS_NO3_CALIB_ZERO:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_NO3, 0x06, 0x1000, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
            
        case _RS485_SS_NO3_CALIB_SLOPE:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_NO3, 0x06, 0x1004, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
            
        case _RS485_SS_NO3_RESET:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_NO3, 0x06, 0x2020, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
...
void Handle_Data_Recv_SS_NO3(sData sDataRS485, uint8_t KindRecv)
{
    uint16_t Pos = 0;
    ...
    switch(KindRecv)
    {
        ...
        case _RS485_SS_NO3_OPERA:
            ...
            sSensor_NO3.NO3_Value_f =  ((float)Stamp_Hex)/(pow(10,Decimal_Hex));
            
            Stamp_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2);
            Decimal_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2); 
            sSensor_NO3.temp_Value_f =  ((float)Stamp_Hex)/(pow(10,Decimal_Hex));
          break;
...
void Handle_State_SS_NO3(uint8_t KindRecv, uint8_t KindDetect)
{
    switch(KindRecv)
    {
        case _RS485_SS_NO3_OPERA:
        ...
        case _RS485_SS_NO3_RESET:
          ...
          {
            sSensor_NO3.Count_Disconnect = 0;
            sSensor_NO3.State_Connect = _SENSOR_CONNECT;
            
            if(KindRecv == _RS485_SS_NO3_OPERA)
              sHandleRs485.State_Recv_NO3 = 1;
          }
          else
          {
            if(sSensor_NO3.Count_Disconnect < 3)
                sSensor_NO3.Count_Disconnect++;
          }
          break;
    ...
    if(sSensor_NO3.Count_Disconnect == 0)
    {
        
    }
    else if(sSensor_NO3.Count_Disconnect >=3)
    {
        sSensor_NO3.State_Connect = _SENSOR_DISCONNECT;
        sSensor_NO3.NO3_Value_f = 0;
        sSensor_NO3.temp_Value_f = 0;

        sHandleRs485.State_Recv_NO3 = 0;
    }
}
/*====================Filter Data===================*/
...
/*===================Save and Init Calib====================*/
void Save_ParamCalib(float NO3_Offset_f, float temp_Offset_f)
{
#ifdef USING_APP_SENSOR
    uint8_t aData[50] = {0};
    uint8_t length = 0;
  
    uint32_t hexUint_Compensation_NO3 = 0;
    uint32_t hexUint_Compensation_temp = 0;
    
    sSensor_NO3.NO3_Offset_f = NO3_Offset_f;
    sSensor_NO3.temp_Offset_f = temp_Offset_f;
    
    hexUint_Compensation_NO3    = Handle_Float_To_hexUint32(sSensor_NO3.NO3_Offset_f);
    hexUint_Compensation_temp   = Handle_Float_To_hexUint32(sSensor_NO3.temp_Offset_f);
    
    aData[length++] = hexUint_Compensation_NO3 >> 24;
    aData[length++] = hexUint_Compensation_NO3 >> 16;
    aData[length++] = hexUint_Compensation_NO3 >> 8;
    aData[length++] = hexUint_Compensation_NO3 ;
...
void Init_ParamCalib(void)
{
    ...
    {
        hexUint_Compensation_NO3  = *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+2) << 24;
        hexUint_Compensation_NO3  |= *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+3)<< 16;
        hexUint_Compensation_NO3  |= *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+4)<< 8;
        hexUint_Compensation_NO3  |= *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+5);
        ...
        Convert_uint32Hex_To_Float(hexUint_Compensation_NO3,  &sSensor_NO3.NO3_Offset_f );
        Convert_uint32Hex_To_Float(hexUint_Compensation_temp, &sSensor_NO3.temp_Offset_f);
    }
    else
    {
        sSensor_NO3.NO3_Offset_f = 0;
        sSensor_NO3.temp_Offset_f = 0;
    }
#endif   
}
