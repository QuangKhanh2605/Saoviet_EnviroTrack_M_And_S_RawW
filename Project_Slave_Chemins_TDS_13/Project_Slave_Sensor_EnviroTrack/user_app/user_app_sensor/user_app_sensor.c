...
static uint8_t fevent_temp_alarm(uint8_t event)
{
    ...
    {
        if((sSensor_EC.TDS_Filter_f >= sTempAlarm.Alarm_Upper) || 
           (sSensor_EC.TDS_Filter_f < sTempAlarm.Alarm_Lower))
        {
...
/*===================Save and Init Calib====================*/
void Save_ParamCalib(float TDS_Offset_f, float temp_Offset_f)
{
    ...
    sSensor_EC.TDS_Offset_f = TDS_Offset_f;
    sSensor_EC.temp_Offset_f = temp_Offset_f;
    
    hexUint_Compensation_EC    = Handle_Float_To_hexUint32(sSensor_EC.TDS_Offset_f);
...
void Init_ParamCalib(void)
{
    ...
    if(*(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE) != FLASH_BYTE_EMPTY)
    {
        ...
        Convert_uint32Hex_To_Float(hexUint_Compensation_EC,  &sSensor_EC.TDS_Offset_f );
        Convert_uint32Hex_To_Float(hexUint_Compensation_temp, &sSensor_EC.temp_Offset_f);
    }
    else
    {
        sSensor_EC.TDS_Offset_f = 0;
