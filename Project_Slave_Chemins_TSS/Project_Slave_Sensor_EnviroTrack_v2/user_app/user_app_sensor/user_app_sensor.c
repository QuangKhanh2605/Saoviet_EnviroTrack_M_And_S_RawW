...
Struct_Sensor_TSS   sSensor_TSS={0};
...
/*------------------Xu ly chuoi nhan duoc----------------*/
    ...
    if(sUart485SS.Length_u16 > 2)
    {
        ...
        if(Crc_Check == Crc_Recv)
        {
            ...
            if(sUart485SS.Data_a8[0] == ID_DEFAULT_SS_TSS)
            {
...
static uint8_t fevent_handle_state_sensor(uint8_t event)
{
    if(sSensor_TSS.State_Connect == _SENSOR_DISCONNECT)
        sSensor_TSS.State_Sensor_u8 = _SS_DISCONNECT;
    else
    {
        if((sLCD.sScreenNow.Index_u8 == _LCD_SCR_SET_CALIB_SS_TSS) || 
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_CHECK_SETTING && sLCD.sScreenBack.Index_u8 == _LCD_SCR_SET_CALIB_SS_TSS) ||
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_SET_CALIB_SS_TSS_PLS) || 
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_CHECK_SETTING && sLCD.sScreenBack.Index_u8 == _LCD_SCR_SET_CALIB_SS_TSS_PLS))
            sSensor_TSS.State_Sensor_u8 = _SS_CALIB;
        else
            sSensor_TSS.State_Sensor_u8 = _SS_MEASURE;
    }
    
    if(sHandleRs485.State_Recv_TSS == 1)
    {
        sSensor_TSS.State_Measure_TSS_u8 = _MEASURE_VALID;
        sSensor_TSS.State_Measure_Temp_u8 = _MEASURE_VALID;
    }
    else
    {
        sSensor_TSS.State_Measure_TSS_u8 = _MEASURE_INVALID;
        sSensor_TSS.State_Measure_Temp_u8 = _MEASURE_INVALID;
    }
...
void Handle_Data_Measure(uint8_t KindRecv)
{
    ...
    switch(KindRecv)
    {
        case _RS485_SS_TSS_OPERA:
            sSensor_TSS.TSS_Value_f = quickSort_Sampling_Value((int32_t)(sSensor_TSS.TSS_Value_f*100));
            
            sCalibPlus.var_x_f = sSensor_TSS.TSS_Value_f;
            sCalibPlus.var_y_f = Cal_CalibPlus(sCalibPlus.var_x_f);
            sSensor_TSS.TSS_Value_f = sCalibPlus.var_y_f;
            
            sSensor_TSS.TSS_Filter_f = Filter_pH(sSensor_TSS.TSS_Value_f);
            sSensor_TSS.temp_Filter_f = Filter_Temp(sSensor_TSS.temp_Value_f);
            
            Stamp = sSensor_TSS.temp_Filter_f + sSensor_TSS.temp_Offset_f;
            sSensor_TSS.temp_Filter_f = ((Stamp) > sMeasureRange.Upper_Temp) ? sMeasureRange.Upper_Temp : ((Stamp) < sMeasureRange.Lower_Temp ? sMeasureRange.Lower_Temp : (Stamp));
            
            Stamp = sSensor_TSS.TSS_Filter_f + sSensor_TSS.TSS_Offset_f;
            sSensor_TSS.TSS_Filter_f = ((Stamp) > sMeasureRange.Upper_Key) ? sMeasureRange.Upper_Key : ((Stamp) < sMeasureRange.Lower_Key ? sMeasureRange.Lower_Key : (Stamp));
          break;
    ...
    if(sSensor_TSS.State_Connect == _SENSOR_DISCONNECT)
    {
        sSensor_TSS.TSS_Value_f = 0;
        sSensor_TSS.TSS_Filter_f = Filter_pH(sSensor_TSS.TSS_Value_f);
    }
    
    if(sSensor_TSS.State_Connect == _SENSOR_DISCONNECT)
    {
        sSensor_TSS.temp_Value_f = 0;
        sSensor_TSS.temp_Filter_f = Filter_Temp(sSensor_TSS.temp_Value_f);;
    }
}
...
void Handle_Data_Trans_SS_TSS(sData *sFrame, uint8_t KindTrans)
{
    switch(KindTrans)
    {
        //Trans Opera
        case _RS485_SS_TSS_OPERA:
            ModRTU_Master_Read_Frame(sFrame, ID_DEFAULT_SS_TSS, 0x03, 0x0000, 0x04);
            break;
            
        case _RS485_SS_TSS_ZERO:
            ModRTU_Master_Read_Frame(sFrame, ID_DEFAULT_SS_TSS, 0x03, 0x1000, 0x01);
            break;
            
        case _RS485_SS_TSS_SLOPE:
            ModRTU_Master_Read_Frame(sFrame, ID_DEFAULT_SS_TSS, 0x03, 0x1004, 0x01);
            break;
            
         //Trans Calib
        case _RS485_SS_TSS_CALIB_ZERO:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_TSS, 0x06, 0x1000, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
            
        case _RS485_SS_TSS_CALIB_SLOPE:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_TSS, 0x06, 0x1004, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
            
        case _RS485_SS_TSS_RESET:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_TSS, 0x06, 0x2020, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
...
void Handle_Data_Recv_SS_TSS(sData sDataRS485, uint8_t KindRecv)
{
    ...
    switch(KindRecv)
    {
        ...
        case _RS485_SS_TSS_OPERA:
            ...
            sSensor_TSS.TSS_Value_f =  ((float)Stamp_Hex)/(pow(10,Decimal_Hex));
            
            Stamp_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2);
            Decimal_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2); 
            sSensor_TSS.temp_Value_f =  ((float)Stamp_Hex)/(pow(10,Decimal_Hex));
          break;
          
        case _RS485_SS_TSS_ZERO:
            Pos = 3;
            Stamp_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2);
            sSensor_TSS.TSS_Zero_Calib_f = ((float)Stamp_Hex)/10;
          break;
          
        case _RS485_SS_TSS_SLOPE:
            Pos = 3;
            Stamp_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2);
            sSensor_TSS.TSS_Slope_Calib_f = ((float)Stamp_Hex)/10;
          break;
...
void Handle_State_SS_TSS(uint8_t KindRecv, uint8_t KindDetect)
{
    switch(KindRecv)
    {
        ...
        case _RS485_SS_TSS_RESET:
          ...
          {
            sSensor_TSS.Count_Disconnect = 0;
            sSensor_TSS.State_Connect = _SENSOR_CONNECT;
            
            if(KindRecv == _RS485_SS_TSS_OPERA)
              sHandleRs485.State_Recv_TSS = 1;
          }
          else
          {
            if(sSensor_TSS.Count_Disconnect < 3)
                sSensor_TSS.Count_Disconnect++;
          }
          break;
    ...
    if(sSensor_TSS.Count_Disconnect == 0)
    {
        
    }
    else if(sSensor_TSS.Count_Disconnect >=3)
    {
        sSensor_TSS.State_Connect = _SENSOR_DISCONNECT;
        sSensor_TSS.TSS_Value_f = 0;
        sSensor_TSS.temp_Value_f = 0;
        
        sSensor_TSS.TSS_Zero_Calib_f = 0;
        sSensor_TSS.TSS_Slope_Calib_f = 0;

        sHandleRs485.State_Recv_TSS = 0;
    }
}
/*====================Filter Data===================*/
...
void Save_ParamCalib(float TSS_Offset_f, float temp_Offset_f)
{
    ...
    sSensor_TSS.TSS_Offset_f = TSS_Offset_f;
    sSensor_TSS.temp_Offset_f = temp_Offset_f;
    
    hexUint_Compensation_TSS = Handle_Float_To_hexUint32(sSensor_TSS.TSS_Offset_f);
    hexUint_Compensation_temp = Handle_Float_To_hexUint32(sSensor_TSS.temp_Offset_f);
...
void Init_ParamCalib(void)
{
    ...
    {
        hexUint_Compensation_TSS  = *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+2) << 24;
        hexUint_Compensation_TSS  |= *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+3)<< 16;
        hexUint_Compensation_TSS  |= *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+4)<< 8;
        hexUint_Compensation_TSS  |= *(__IO uint8_t*)(ADDR_CALIB_TEMPERATURE+5);
        ...
        Convert_uint32Hex_To_Float(hexUint_Compensation_TSS,  &sSensor_TSS.TSS_Offset_f);
        Convert_uint32Hex_To_Float(hexUint_Compensation_temp, &sSensor_TSS.temp_Offset_f);
    }
    else
    {
        sSensor_TSS.TSS_Offset_f = 0;
        sSensor_TSS.temp_Offset_f = 0;
    }
#endif   
}
...
void Init_CalibPlus(void)
{
    ...
    {
        sCalibPlus.PZero.Mode = 0;
        sCalibPlus.PZero.pt_x = 0;
        sCalibPlus.PZero.pt_y = 0;
        ...
        sCalibPlus.PSlope.Mode = 0;
        sCalibPlus.PSlope.pt_x = 0;
        sCalibPlus.PSlope.pt_y = 0;
    }
#endif   
}
