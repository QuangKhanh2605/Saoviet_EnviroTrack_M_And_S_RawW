...
static uint8_t fevent_handle_state_sensor(uint8_t event)
{
    if(sSensor_pH.State_Connect == _SENSOR_DISCONNECT)
        sSensor_pH.State_Sensor_u8 = _SS_DISCONNECT;
    else
    {
        if((sLCD.sScreenNow.Index_u8 == _LCD_SCR_SET_CALIB_SS_PH) || 
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_CHECK_SETTING && sLCD.sScreenBack.Index_u8 == _LCD_SCR_SET_CALIB_SS_PH) ||
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_SET_CALIB_SS_PH_PLS) || 
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_CHECK_SETTING && sLCD.sScreenBack.Index_u8 == _LCD_SCR_SET_CALIB_SS_PH_PLS))
            sSensor_pH.State_Sensor_u8 = _SS_CALIB;
        else
            sSensor_pH.State_Sensor_u8 = _SS_MEASURE;
    }
    
    if(sHandleRs485.State_Recv_pH == 1)
    {
        sSensor_pH.State_Measure_PH_u8 = _MEASURE_VALID;
        sSensor_pH.State_Measure_Temp_u8 = _MEASURE_VALID;
    }
    else
    {
        sSensor_pH.State_Measure_PH_u8 = _MEASURE_INVALID;
        sSensor_pH.State_Measure_Temp_u8 = _MEASURE_INVALID;
    }
...
void Handle_Data_Measure(uint8_t KindRecv)
{
    ...
    switch(KindRecv)
    {
        case _RS485_SS_PH_OPERA:
            ...
            sCalibPlus.var_x_f = sSensor_pH.pH_Value_f;
            sCalibPlus.var_y_f = Cal_CalibPlus(sCalibPlus.var_x_f);
            sSensor_pH.pH_Value_f = sCalibPlus.var_y_f;
            
            sSensor_pH.pH_Filter_f = Filter_pH(sSensor_pH.pH_Value_f);
            sSensor_pH.temp_Filter_f = Filter_Temp(sSensor_pH.temp_Value_f);
            
            Stamp = sSensor_pH.temp_Filter_f + sSensor_pH.temp_Offset_f;
            sSensor_pH.temp_Filter_f = ((Stamp) > sMeasureRange.Upper_Temp) ? sMeasureRange.Upper_Temp : ((Stamp) < sMeasureRange.Lower_Temp ? sMeasureRange.Lower_Temp : (Stamp));
            
            Stamp = sSensor_pH.pH_Filter_f + sSensor_pH.pH_Offset_f;
            sSensor_pH.pH_Filter_f = ((Stamp) > sMeasureRange.Upper_Key) ? sMeasureRange.Upper_Key : ((Stamp) < sMeasureRange.Lower_Key ? sMeasureRange.Lower_Key : (Stamp));
          break;
...
void Handle_Data_Trans_SS_pH(sData *sFrame, uint8_t KindTrans)
{
    switch(KindTrans)
    {
        //Trans Opera
        case _RS485_SS_PH_OPERA:
            ModRTU_Master_Read_Frame(sFrame, ID_DEFAULT_SS_PH, 0x03, 0x0000, 0x04);
            break;
            
         //Trans Calib
        case _RS485_SS_PH_CALIB_686:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_PH, 0x06, 0x1000, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
            
        case _RS485_SS_PH_CALIB_918:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_PH, 0x06, 0x1004, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
            
        case _RS485_SS_PH_CALIB_400:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_PH, 0x06, 0x1002, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
            
        case _RS485_SS_PH_RESET:
            if(sData_Calib.Length_u16 == 2)
                ModRTU_Master_Write_Frame(sFrame, ID_DEFAULT_SS_PH, 0x06, 0x2020, (sData_Calib.Length_u16/2), sData_Calib.Data_a8);
            break;
...
void Handle_Data_Recv_SS_pH(sData sDataRS485, uint8_t KindRecv)
{
    ...
    switch(KindRecv)
    {
        ...
        case _RS485_SS_PH_OPERA:
            ...
            sSensor_pH.pH_Value_f =  ((float)Stamp_Hex)/(pow(10,Decimal_Hex));
            
            Stamp_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2);
            Decimal_Hex = Read_Register_Rs485(sDataRS485.Data_a8, &Pos, 2); 
            sSensor_pH.temp_Value_f =  ((float)Stamp_Hex)/(pow(10,Decimal_Hex));
          break;
